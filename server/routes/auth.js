...
async function sendOtpEmail(user, otp) {
  try {
    ...
    const from = process.env.EMAIL_FROM || 'HealthHub <noreply@healthhub.com>';
    const to = user.email;
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #7c3aed;">HealthHub Verification</h2>
        <p>Hello ${user.name},</p>
        ...
        <p style="color: #6b7280; font-size: 14px;">HealthHub - Find trusted doctors and book appointments</p>
      </div>
    `;
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    const transporter = await getTransporter();
    const from = process.env.EMAIL_FROM || 'HealthHub <noreply@healthhub.com>';
    const subject = 'Reset your HealthHub password';
    const text = `Click here to reset your password: ${resetUrl}`;
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #7c3aed;">HealthHub Password Reset</h2>
        <p>Hello ${user.name},</p>
